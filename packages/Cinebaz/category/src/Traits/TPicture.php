...
namespace Cinebaz\Category\Traits;

use Cinebaz\Category\Models\Category;
use Cinebaz\Category\Models\CategoryPicture;
...
trait TPicture
{
    ...
    public function imgUpload($data)
    {
        ...
        if($image){
            ...
            $data = 'storage/images/dropzon/category/' . $imageName;

            $image->move(public_path('storage/images/dropzon/category'), $imageName);

            if ($request->id) {
                $media = Category::find($request->id);
                if($media->allimages){
                    $media->allimages()->delete();
                }
            }
        ...
        if ($request->image) {
            ...
            CategoryPicture::create($attributes);
            //dd($attributes);
        }
    }
}
